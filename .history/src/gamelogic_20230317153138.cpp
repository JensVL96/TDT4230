...
unsigned int loadCubemap(std::string path) {
    ...
    std::string files[] = {"left.jpg", "right.jpg", "top.jpg", "bottom.jpg", "front.jpg", "wood.jpg"};

    for (unsigned int i = 0; i < 6; i++) {
        std::cout << (path + files[i]) << std::endl;
        auto texture = loadPNGFile(path + files[i]);
        glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, GL_RGB, texture.width, texture.height, 0, GL_RGB, GL_UNSIGNED_BYTE, texture.pixels.data());
        }
...
void initGame(GLFWwindow* window, CommandLineOptions gameOptions) {
    ...
    skybox = new Gloom::Shader();
    shader->makeBasicShader("../res/shaders/skybox.vert", "../res/shaders/skybox.frag");
    shader->activate();
    ...
    // boxNode->textureID            = brick_col.ID;
    // boxNode->normalMapID          = brick_nrm.ID;
    boxNode->textureID            = loadCubemap("../res/skybox/");
        ...
        case TEXTURE_CUBE_MAP: 
            if(node->vertexArrayObjectID != -1) {
                skybox.activate()
                    ...
                    glBindVertexArray(node->vertexArrayObjectID);
                    glBindTexture(2, node->textureID);
                    // glUniform1i(12, 1);
                    glDrawElements(GL_TRIANGLES, node->VAOIndexCount, GL_UNSIGNED_INT, nullptr);
            }
        break;
